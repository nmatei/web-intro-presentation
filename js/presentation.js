var Presentation = (function(){
	var params = window.location.search.substring(1).split("&").reduce(function(prev, curr, i, arr) {
		var p = curr.split("=");
...
(function(){
	...
	animElementsItems.push(
		'<a href="?print" class="btn' + (isPrintMode ? ' present' : '') + '" title="Print Mode: use print as PDF in Chrome (margins: 0.15in 0 0.18in 0)">',
	...
	if(document.body.classList.contains('pdf-available')) {
		...
		animElementsItems.push(
			'<a href="pdf/', pdfPath, '" target="_blank" class="btn" title="PDF Version">',
	...
	animElementsItems.push(
		...
		'</a>',
		'<a href="?anim=slide-left" title="Slide Right/Left" class="btn' + (animation === 'slide-left' ? ' present' : '') + '">',
	...
	for (var i = 0; i < length; i++) {
		...
		tocContent.push('<a id="toc-' + page.id + '" href="#'+ (isPrintMode ? '' : '/') + page.id + '" title="' + page.id + '">' + ( i + 1 ) + '</a>');
